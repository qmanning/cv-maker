# Installing CV Maker — a checklist for AI assistants

You are setting up **CV Maker**, a prebuilt static tool (one folder of HTML/CSS/JS, plus a
...
- **Do not modify files in the `cv-maker/` folder** except `config.js` and anything under
...
- **Never run `npm install` or `npm run build` inside the user's project for this tool.** Those
  commands are only for developing CV Maker itself, from a clone of its own repo.
...
Infospector, CV Maker does **not** need to share an origin with anything else — it doesn't frame
the user's pages — so it can live at any path your server serves, at any depth.

## 2. Copy the folder

Copy the whole `cv-maker/` folder to `<static dir>/labs/cv-maker/` (create `labs/`). Keep the
folder name `cv-maker`. Result must contain at least: `index.html`, `config.js`, `dist/cv-maker.js`,
`dist/cv-maker.css`, `dist/chunks/`, `templates/sample-resume.html`, `vendor/html-to-image.js`,
`vendor/infospector/host.css`, `vendor/infospector/lib.js`, `vendor/infospector/colorpicker.js`,
`README.md`, `LICENSE`.

CV Maker must be served over `http://` or `https://` — opening `index.html` via `file://` will not
...
- `GET <origin>/labs/cv-maker/index.html` → 200, `text/html`
- `GET <origin>/labs/cv-maker/dist/cv-maker.js` → 200, `application/javascript` (or `text/javascript`)
- `GET <origin>/labs/cv-maker/dist/cv-maker.css` → 200, `text/css`

If you have a browser tool: open `<origin>/labs/cv-maker/`, wait for at least one
...
Checklist for turning their existing résumé into a CV Maker source file:
...
Edit `<static dir>/labs/cv-maker/config.js` only — it's already there with commented examples:
...
one-line reminder that Export → Source HTML is how they keep named variants — CV Maker only
...
| 404 on `dist/chunks/*.js` | The folder was copied partially — re-copy the whole `cv-maker/` folder, `dist/` included |
