// electron/mcp/catalog.mjs — WHAT IcedCoffee offers an AI app: the instructions, the tools, and how each tool maps onto the app.
...
//     always gets the tools of the IcedCoffee that is actually installed and running;
//   • by the stdio server (server.mjs) only as a fallback for when IcedCoffee isn't open yet.
...
export const INSTRUCTIONS = `IcedCoffee is a desktop editor for a résumé AND its cover letter; these tools act on what the person has open in it right now, and they watch the page change as you work.

Two documents: IcedCoffee holds a résumé and a cover letter side by side (two tabs, two files). Every document tool REQUIRES "document": "resume" or "cover_letter" — always name which one; the two are separate files and there is no default, so an edit can never land on the wrong one. Ids (b0, r0, i0 …) belong to ONE document: call get_document with the same "document" before editing it. The cover letter's header is a read-only copy of the résumé's header — change the name, contact details or headline on the résumé and the letter follows; the letter's own regions are its date, recipient, greeting, body and sign-off.
...
Importing: open_document with an absolute path to a Word (.docx), PDF, RTF, Markdown (.md) or plain-text (.txt) file — or an HTML résumé not made in IcedCoffee — converts it with IcedCoffee's own rules (no AI involved): the look (fonts, sizes, colours, spacing, bullets, tables, columns, pictures) and the structure (name and contact, sections, dated entries) come across as a new, unsaved document. The result says what was found and what could not come across. A scanned PDF has no text to read: say so to the person rather than guessing its contents.
...
export async function callTool(name, args, app) {
    ...
    if (name === "edit_document") {
        ...
        return text({ ...out, note: spilled ? `It now runs to ${out.pages_after} pages (it was ${out.pages_before}). Unless the person wants that, tighten what you just wrote and call edit_document again.` : out.applied ? "Applied. The person can see it and has an Undo button in IcedCoffee." : "Nothing was applied; see skipped." });
    }
