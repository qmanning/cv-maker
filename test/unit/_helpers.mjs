// test/unit/_helpers.mjs — shared helpers for CV Maker's node:test unit suite.
...
async function importBundleText(text) {
    const tmpFile = path.join(os.tmpdir(), `cv-maker-test-${process.pid}-${Date.now()}-${Math.random().toString(36).slice(2)}.mjs`);
