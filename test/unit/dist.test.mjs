// Sanity/portability checks on the COMMITTED build (dist/icedcoffee.js, dist/icedcoffee.css, dist/chunks/*)
...
const jsPath = path.join(distDir, "icedcoffee.js");
const cssPath = path.join(distDir, "icedcoffee.css");
...
test("dist/icedcoffee.js and dist/icedcoffee.css exist and are non-trivial in size", () => {
    assert.ok(fs.statSync(jsPath).size > 50_000, "icedcoffee.js seems too small to be the real bundle");
    assert.ok(fs.statSync(cssPath).size > 2_000, "icedcoffee.css seems too small to be the real stylesheet");
});

test("every chunk icedcoffee.js imports (static or dynamic) exists on disk, relative to dist/", () => {
    const specifiers = new Set();
    for (const m of js.matchAll(/from"(\.[^"]+)"/g)) specifiers.add(m[1]);
    for (const m of js.matchAll(/from'(\.[^']+)'/g)) specifiers.add(m[1]);
    for (const m of js.matchAll(/import\("(\.[^"]+)"\)/g)) specifiers.add(m[1]);
    for (const m of js.matchAll(/import\('(\.[^']+)'\)/g)) specifiers.add(m[1]);
    assert.ok(specifiers.size > 0, "expected at least one relative import/chunk specifier in icedcoffee.js");
    for (const spec of specifiers) {
        const resolved = path.join(distDir, spec);
        assert.ok(fs.existsSync(resolved), `chunk "${spec}" imported by icedcoffee.js does not exist at ${resolved}`);
    }
});

test("dist files contain no absolute filesystem paths from this machine", () => {
    for (const [name, text] of [["icedcoffee.js", js], ["icedcoffee.css", css]]) {
...
test("dist files contain none of the privacy-guard strings", () => {
    for (const [name, text] of [["icedcoffee.js", js], ["icedcoffee.css", css]]) {
        // the tool credits its author in the ⋯ menu ("IcedCoffee by Q Manning" → qmanning.com/labs/icedcoffee) — that is
        ...
        const withoutCredit = text.replaceAll("https://qmanning.com/labs/icedcoffee", "").replaceAll("by Q Manning", "").replace(/qmanning\.com (?:\u2197|\\u2197)/g, "");
...
test("config.js parses as valid JS and, with its commented examples left commented, only sets window.ICEDCOFFEE = {}", () => {
    ...
    assert.deepEqual(sandbox.window.ICEDCOFFEE, {});
});
